...
app.post('/api/login', (req, res) => {
  ...
  console.log('Query parameters:', [usernameOrEmail, usernameOrEmail]); // Parola nu mai este inclusă aici în query
  ...
  db.get(sql, [usernameOrEmail, usernameOrEmail], (err, row) => {
    ...
    if (row) {
      // Utilizatorul a fost găsit, acum verifică parola
...
app.post('/api/add-to-profile/:userId', (req, res) => {
  ...
  //This needs to be gotten by local storage and get value via that, and add to the front end.
  // Get the user id from front end.
  const userId = req.params.userId; //To properly get.
